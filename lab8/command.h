...
inline void createStudent(vector<Student*>& students) {
	int count, age, id;
...
inline void createTeacher(vector<Teacher*> &teachers) {
	int age;
...
inline void crateCourse(vector<Teacher*> &teachers) {
	try
	{	
		if (teachers.empty())
		{
...
inline void crateDepartment(vector<Teacher*> &teachers, Department<string,Course> &department) {
	if (!teachers.empty())
	{
...
inline void show_student_array(vector<Student*>& students) {
	try {
		if (students.empty())
		{
...
inline void show_teacher_array(vector<Teacher*> &teachers) {
	if (!teachers.empty())
	{
...
inline void show_course(vector<Teacher*> &teachers) {
	if (!teachers.empty())
	{
		cout << "Виберiть викладача для виводу iнформацiї" << endl;
		int num;
...
inline void show_Department(Department<string, Course> &department) {
...
inline void set_grades(vector<Teacher*>& teachers, vector<Student*>& students) {
		try
		{
			if (students.empty())
			{
...
inline void subscription_on_course(vector<Student*>& students, vector<Teacher*>& teachers) {
...
inline void unsubscribe_on_course(vector<Student*>& students, vector<Teacher*>& teachers) {
...
inline void writeStudentsToFile(vector<Student*>& students, string filename) {
	try {
...
inline void readStudentsFromFile(vector<Student*>& students, string filename) {
	try {
			...
			if (line == "Студент") {
				...
				*student += stoi(rating);
				...
				while(1) 
				{
					...
					if (pos != string::npos) {
						string subject = line.substr(1, pos - 1);
...
inline void writeTeacherToFile(vector<Teacher*>& teachers, string filename) {
	ofstream file(filename, ios::trunc);
	if (!file.is_open()) {
...
inline void readTeachersFromFile(vector<Teacher*>& teachers, vector<Student*>& students, string filename1, string filename2) {
	try {
...
inline void saveCoursesToFile(vector<Teacher*>& teachers, string filename) {
	ofstream file(filename, ios::trunc);
	if (!file.is_open()) {
...
inline void sortStudentsByRating(vector<Student*>& students) {
...
template <typename T>
inline void delete_item(vector<T*>& items, int num) {
	try {
		if (items.empty()) {
...
inline void delete_course(vector<Teacher*>& teachers) {
...
inline void rating(vector<Student*>& students, vector<Teacher*>& teachers) {
	try{
		if (teachers.empty())
		{
